...
{
	this->createVAO(Model::ModelType::CameraModel, 0, 0);
}

void CameraModel::createVAO(Model::ModelType type, const char * const modelFileName, int textureNumber)
{
	shutUp(type);
...
#define SPHERE_DATA_NUM_VERTS (sizeof(pVerts)/sizeof(Vert_xyzuvn))
	...
	Camera *pCam = CameraManager::Find(Camera::Name::CAMERA_0);
	pCam->updateCamera();
...
void CameraModel::createVAO(Model::ModelType type, const char * const modelFileName, const char * const objectName)
{
	shutUp(type);
	shutUp(modelFileName);
	shutUp(objectName);
...
CameraModel::CameraModel(const char * const modelFileName)
	: Model()
{
	this->createVAO(Model::ModelType::CameraModel, modelFileName, 0);
}
